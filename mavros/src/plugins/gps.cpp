...
void GPSInfo::set_gps_raw(mavlink_gps_raw_int_t &gps) {
...
void GPSInfo::run(diagnostic_updater::DiagnosticStatusWrapper &stat) {
...
void GPSPlugin::initialize(UAS &uas_,
		ros::NodeHandle &nh,
...
std::string const GPSPlugin::get_name() const {
	return "GPS";
}

const MavRosPlugin::message_map GPSPlugin::get_rx_handlers() {
	return {
...
void GPSPlugin::handle_gps_raw_int(const mavlink_message_t *msg, uint8_t sysid, uint8_t compid) {
...
void GPSPlugin::handle_gps_status(const mavlink_message_t *msg, uint8_t sysid, uint8_t compid) {
