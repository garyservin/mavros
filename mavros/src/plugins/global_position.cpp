...
GlobalPositionPlugin::GlobalPositionPlugin() :
		uas(nullptr),
		send_tf(false),
		rot_cov(99999.0)
	{ };

void GlobalPositionPlugin::initialize(UAS &uas_,
		ros::NodeHandle &nh,
...
std::string const GlobalPositionPlugin::get_name() const {
	return "GlobalPosition";
}

const MavRosPlugin::message_map GlobalPositionPlugin::get_rx_handlers() {
	return {
		MESSAGE_HANDLER(MAVLINK_MSG_ID_GLOBAL_POSITION_INT, &GlobalPositionPlugin::handle_global_position_int)
	};
}

void GlobalPositionPlugin::handle_global_position_int(const mavlink_message_t *msg, uint8_t sysid, uint8_t compid) {
