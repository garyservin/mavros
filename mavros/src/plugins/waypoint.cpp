...
mavros::Waypoint WaypointItem::to_msg(WaypointItem &wp) {
	mavros::Waypoint ret;
...
WaypointItem WaypointItem::from_msg(mavros::Waypoint &wp, uint16_t seq) {
	WaypointItem ret;
...
WaypointItem WaypointItem::from_mission_item(mavlink_mission_item_t &mit) {
	WaypointItem ret;
...
std::string WaypointItem::to_string_frame(WaypointItem &wpi) {
	switch (wpi.frame) {
...
std::string WaypointItem::to_string_command(WaypointItem &wpi) {
	switch (wpi.command) {
...
void WaypointPlugin::initialize(UAS &uas_,
		ros::NodeHandle &nh,
...
std::string const WaypointPlugin::get_name() const {
	return "Waypoint";
};

const MavRosPlugin::message_map WaypointPlugin::get_rx_handlers() {
	return {
...
void WaypointPlugin::handle_mission_item(const mavlink_message_t *msg, uint8_t sysid, uint8_t compid) {
...
void WaypointPlugin::handle_mission_request(const mavlink_message_t *msg, uint8_t sysid, uint8_t compid) {
...
void WaypointPlugin::handle_mission_current(const mavlink_message_t *msg, uint8_t sysid, uint8_t compid) {
...
void WaypointPlugin::handle_mission_count(const mavlink_message_t *msg, uint8_t sysid, uint8_t compid) {
...
void WaypointPlugin::handle_mission_item_reached(const mavlink_message_t *msg, uint8_t sysid, uint8_t compid) {
...
void WaypointPlugin::handle_mission_ack(const mavlink_message_t *msg, uint8_t sysid, uint8_t compid) {
...
void WaypointPlugin::timeout_cb(const ros::TimerEvent &event) {
	unique_lock lock(mutex);
	if (wp_retries > 0) {
		wp_retries--;
...
void WaypointPlugin::connection_cb(bool connected) {
	lock_guard lock(mutex);
	if (connected)
...
void WaypointPlugin::sheduled_pull_cb(const ros::TimerEvent &event) {
	lock_guard lock(mutex);
	if (wp_state != WP_IDLE) {
		/* try later */
...
void WaypointPlugin::request_mission_done(void) {
...
void WaypointPlugin::go_idle(void) {
	reshedule_pull = false;
	wp_state = WP_IDLE;
	wp_timer.stop();
}

void WaypointPlugin::restart_timeout_timer(void) {
	wp_retries = RETRIES_COUNT;
	restart_timeout_timer_int();
}

void WaypointPlugin::restart_timeout_timer_int(void) {
	is_timedout = false;
	wp_timer.stop();
	wp_timer.start();
}

void WaypointPlugin::shedule_pull(const ros::Duration &dt) {
	shedule_timer.stop();
	shedule_timer.setPeriod(dt);
	shedule_timer.start();
}

void WaypointPlugin::send_waypoint(size_t seq) {
...
bool WaypointPlugin::wait_fetch_all() {
...
bool WaypointPlugin::wait_push_all() {
...
void WaypointPlugin::set_current_waypoint(size_t seq) {
...
void WaypointPlugin::publish_waypoints() {
...
void WaypointPlugin::mission_item(WaypointItem &wp) {
...
void WaypointPlugin::mission_request(uint16_t seq) {
	mavlink_message_t msg;
...
void WaypointPlugin::mission_set_current(uint16_t seq) {
	mavlink_message_t msg;
...
void WaypointPlugin::mission_request_list() {
	mavlink_message_t msg;
...
void WaypointPlugin::mission_count(uint16_t cnt) {
	mavlink_message_t msg;
...
void WaypointPlugin::mission_clear_all() {
	mavlink_message_t msg;
...
void WaypointPlugin::mission_ack(enum MAV_MISSION_RESULT type) {
	mavlink_message_t msg;
...
bool WaypointPlugin::pull_cb(mavros::WaypointPull::Request &req,
...
bool WaypointPlugin::push_cb(mavros::WaypointPush::Request &req,
...
bool WaypointPlugin::clear_cb(mavros::WaypointClear::Request &req,
...
bool WaypointPlugin::set_cur_cb(mavros::WaypointSetCurrent::Request &req,
...
bool WaypointPlugin::goto_cb(mavros::WaypointGOTO::Request &req,
